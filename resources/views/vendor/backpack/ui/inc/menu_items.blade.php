...
<x-backpack::menu-dropdown title="Электронный журнал" icon="la la-school">
    @if(backpack_user()->role_id == 1 || backpack_user()->role_id == 2)
        <x-backpack::menu-dropdown-item title="Журнал преподавателя" icon="la la-question" :link="backpack_url('eljur')"/>
    @endif
    @if(backpack_user()->role_id == 3)
        <x-backpack::menu-dropdown-item title="Журнал студента" icon="la la-question" :link="backpack_url('eljur/student')"/>
    @endif
</x-backpack::menu-dropdown>

<x-backpack::menu-dropdown title="Ведомости" icon="la la-school">
    <x-backpack::menu-dropdown-item title="Ежемесячные ведомости" icon="la la-question"
                                    :link="backpack_url('report-group-month')"/>
    <x-backpack::menu-dropdown-item title="Промежуточные ведомости" icon="la la-question"
...
@if(backpack_user()->role_id != 3)
    <x-backpack::menu-dropdown title="Пользователи" icon="la la-group">
        ...
        @endif
        @if(backpack_user()->role_id == 2 || backpack_user()->role_id == 1)
...
@if(backpack_user()->role_id == 1)
    <x-backpack::menu-dropdown title="Учебный план" icon="la la-school">
        <x-backpack::menu-dropdown-item title="Специальности" icon="la la-drafting-compass" :link="backpack_url('specialty')"/>
        <x-backpack::menu-dropdown-item title="Дисциплины" icon="la la-ruler-combined" :link="backpack_url('subject')"/>
        <x-backpack::menu-dropdown-item title="Темы" icon="la la-book" :link="backpack_url('topic')"/>
        <x-backpack::menu-dropdown-item title="Связи преподавателей и групп" icon="la la-book" :link="backpack_url('teacher-group-subject')" />
    </x-backpack::menu-dropdown>
@endif
...
@if(backpack_user()->role_id == 1)
    <x-backpack::menu-dropdown title="Параметры курсов" icon="la la-cog">
        ...
        <x-backpack::menu-dropdown-item title="Типы лекций" icon="la la-shapes" :link="backpack_url('lesson-type')"/>
    </x-backpack::menu-dropdown>
@endif
